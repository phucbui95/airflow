...
class RedshiftQueryResultToS3Operator(BaseOperator):
    """
        Executes an UNLOAD command to s3 as a CSV with headers

        :param select_query: reference to a select query
        :type schema: str
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        *,
        ...
        table_as_file_name: bool = True,
        # Set to True by default for not breaking current workflows
        **kwargs,
    ) -> None:
        ...
        self.dest_key = self.s3_key
        if self.include_header and 'HEADER' not in [uo.upper().strip() for uo in
                                                    self.unload_options]:
    ...
    def execute(self, context) -> None:
        ...
        credentials_object = s3_hook.get_credentials()
        unload_options = '\n\t\t\t'.join(self.unload_options)
        s3_key = self.dest_key
        credentials = "aws_access_key_id={access_key};" \
                      "aws_secret_access_key={secret_key}".format(
            access_key=credentials_object.access_key,
            secret_key=credentials_object.secret_key,
        )

        if credentials_object.token:
            credentials += ";token={}".format(credentials_object.token)
...
class RedshiftToS3Operator(RedshiftQueryResultToS3Operator):
    """
    Executes an UNLOAD command to s3 as a CSV with headers
